...
export interface ProgressiveFluxLoaderProps {
   ...
   */
  gradient?: string;
  /** Optional custom box shadow for the bar glow. */
  shadow?: string;
...
const BAR_SHADOW = `0 0 14px color-mix(in oklab, ${FLUX_FROM} 55%, transparent), 0 0 24px color-mix(in oklab, ${FLUX_TO} 40%, transparent), inset 0 1px 0 rgba(255, 255, 255, 0.5), inset 0 -1.5px 2px color-mix(in oklab, ${FLUX_FROM} 80%, black)`;
...
function FluxLabel({ label, reduced, className }: FluxLabelProps) {
  const base = cn(
    "absolute inset-0 flex items-center justify-center text-center font-bold tracking-widest text-muted-foreground",
...
export function ProgressiveFluxLoader({
  ...
  return (
    <div
      className={cn(
        "mx-auto flex w-full max-w-sm flex-col items-center gap-5",
        className,
      )}
    >
      {showLabel && (
      ...
      <div
        className={cn(
          "relative h-2.5 w-full overflow-hidden rounded-full bg-muted shadow-[inset_0_1.5px_2px_rgba(0,0,0,0.15)] dark:shadow-[inset_0_2px_3px_rgba(0,0,0,0.45)]",
          barClassName,
        )}
        role="progressbar"
        aria-valuemin={0}
